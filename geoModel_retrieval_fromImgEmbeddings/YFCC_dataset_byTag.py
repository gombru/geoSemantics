...
class YFCC_Dataset(Dataset):
    def __init__(self, root_dir, split, img_backbone_model):
        ...
        if 'train' in self.split:
            ...
            self.num_elements = 1024 * 200  # 1024 * 100
        else:
        ...
        # print("Normalizing vocab [0-1]")
        # vocab_min = 4.2095
        # vocab_max = 4.1901
        # for k, v in self.text_model.items():
        #     v = np.asarray(v, dtype=np.float32)
        #     self.text_model[k] = (v + vocab_min) / (vocab_min + vocab_max)
        ...
        print("Reading image embeddings (and L2 normalizing)")
        img_em_c = 0
        ...
        for i, line in enumerate(open(self.img_embeddings_path)):
            ...
            # img_em = (img_em + img_min) / (img_max + img_min)
    ...
    def __getitem__(self, idx):
        ...
        # Get tag
        checking_tag_image = True
        # Check there are images for this tag, else pick another one
        while checking_tag_image:
            tag_str = self.tags_list[idx]
            img_with_cur_tag = self.images_per_tag[tag_str]
            num_img_with_cur_tag = len(img_with_cur_tag)
            if isinstance(img_with_cur_tag, list) and num_img_with_cur_tag > 3:
                tag = self.__getwordembedding__(tag_str)
                checking_tag_image = False
            else:
                idx = random.randint(0, self.num_elements - 1)

        # Select a random image containing the tag
        img_p_id = random.choice(img_with_cur_tag)
        img_p_idx = self.img_ids2idx_map[img_p_id]

        try:
            img_p = self.img_embeddings[img_p_id]
        except:
            print("Couldn't find img embedding for image: " + str(self.img_ids[idx]) + ". Using 0s. " + str(idx))
            img_p = np.zeros(300, dtype=np.float32)

        # Get lat lon
        lat = self.latitudes[img_p_idx]
        lon = self.longitudes[img_p_idx]
        ...
        if negative_type == 0:  # Select a random negative
            img_n_index = self.__getItemNotSharingTag__(img_p_idx, tag_str)
            ...
            while True:
                ...
                if img_n_index != img_p_idx:
                    # Check that the distance is above distance threshold
                    dist_checked += 1
                    locations_distance = self.__getdistanceFast__(self.latitudes_or[img_p_idx], self.longitudes_or[img_p_idx],
        ...
        try:
            ...
            print("Couldn't find img embedding for image: " + str(self.img_ids[img_n_index]) + ". Using 0s. " + str(img_n_index))
            img_p = np.zeros(300, dtype=np.float32)

        # print("lat: " + str(lat) + "img_p: " + str(img_p[0:3]) + " img_n: " + str(img_n[0:5]))
        # Build tensors
