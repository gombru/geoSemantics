...
def train(train_loader, model, criterion, optimizer, epoch, print_freq, plot_data):
    ...
    for i, (image, label) in enumerate(train_loader):
        ...
        prec1_meter.update(prec1, image.size()[0])
        prec5_meter.update(prec5, image.size()[0])
        ...
        if i % print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec1 {prec1_meter.val:.3f} ({prec1_meter.avg:.3f})\t'
                  'Prec5 {prec5_meter.val:.3f} ({prec5_meter.avg:.3f})\t'
                .format(
