...
def train(train_loader, model, criterion, optimizer, epoch, print_freq, plot_data):
    ...
    for i, (img_p, img_n, tag, lat, lon) in enumerate(train_loader):
        ...
        if i % 5000 == 0 and i != 0:
            filename = '../../../hd/datasets/YFCC100M/' + '/models/' + 'geoModel_retrieval_CNN_NCSL_frozen_noLoc_M1_NotNorm_withProgressiveLocation' + '_epoch_' + str(epoch) + '_iter_' + str(i) + '_TrainLoss_' + str(round(loss.data.item(), 2))
