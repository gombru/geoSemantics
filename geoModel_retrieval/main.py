...
# Config
training_id = 'geoModel_retrieval_BB_NCSL_randomTriplets_M1'
dataset = '../../../datasets/YFCC100M/'
split_train = 'train_filtered.txt'
split_val = 'val.txt'

CNN_checkpoint = 'YFCC_NCSL_3rdtraining_epoch_3_ValLoss_0.37.pth'
CNN_checkpoint = dataset + 'models/saved/' + CNN_checkpoint
...
for epoch in range(start_epoch, epochs):
    ...
    # Train for one epoch
    plot_data = train.train(train_loader, model, criterion, optimizer, epoch, print_freq, plot_data, gpu, margin)

    # Evaluate on validation set
    plot_data = train.validate(val_loader, model, criterion, epoch, print_freq, plot_data, gpu, margin)
