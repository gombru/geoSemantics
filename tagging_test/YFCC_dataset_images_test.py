...
class YFCC_Dataset_Images_Test(Dataset):
    def __init__(self, root_dir, split, central_crop=224):
        ...
        self.num_elements = sum(1 for line in open(split))
        ...
        # Read data
        print("Reading data ...")
        for i, line in enumerate(open(split)):
            if i % 2000000 == 0 and i != 0: print(i)
            data = line.split(';')
            self.img_ids.append(data[0].rstrip().split('/')[-1])
    ...
    def __getitem__(self, idx):
        img_name = '{}/{}'.format(self.root_dir, self.img_ids[idx])
